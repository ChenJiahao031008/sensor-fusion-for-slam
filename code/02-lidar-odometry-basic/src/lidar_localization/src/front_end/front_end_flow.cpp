...
bool FrontEndFlow::ReadData() {
    ...
    // std::deque是双端队列，可以高效的在头尾两端插入和删除元素,比vector更加高效
    // 函数内的static类似于全局变量
    static std::deque<IMUData> unsynced_imu_;
    static std::deque<GNSSData> unsynced_gnss_;
    // 不同于点云数据，imu和gnss数据需要同步处理，因此没有直接赋值到全局变量中
    ...
    // unsynced_imu_是没有同步过得imu数据，imu_data_buff_是全局变量，已经处理好的数据
...
bool FrontEndFlow::InitCalibration() {
    ...
    if (!calibration_received) {
        // 这里广播了imu和lidar的变换矩阵 lidar_to_imu_
...
bool FrontEndFlow::UpdateGNSSOdometry() {
    ...
    // 使用gnss的绝对位置信息做平移和imu的旋转做旋转，形成新的坐标系
