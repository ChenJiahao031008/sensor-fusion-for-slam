...
void Activity::Init(void) {
    ...
    // 记录轨迹
    private_nh_.param("file_path", file_path, std::string("/workspace/assignments/06-imu-navigation/src/imu_integration/data/"));

    ground_truth_file = file_path + "gt.txt";
    std::ofstream OutGT(ground_truth_file.c_str());
    if (OutGT.is_open())
        OutGT.close();
}
...
void Activity::SetOdometryMessage(void) {
    ...
    // e. record ground truth
    std::ofstream OutGT(ground_truth_file.c_str(), std::ios::app);
    if (OutGT.is_open())
    {
        OutGT << message_odom_.header.stamp << " " << t_gt_.x() << " " << t_gt_.y() << " " << t_gt_.z() << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << std::endl;
        OutGT.close();
    }
}
