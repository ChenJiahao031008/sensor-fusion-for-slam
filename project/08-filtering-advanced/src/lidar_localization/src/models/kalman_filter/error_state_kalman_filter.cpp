...
void ErrorStateKalmanFilter::GetOdometry(Eigen::Matrix4f &pose,
  ...
  // std::cout << "V_B_y : " << vel_double << std::endl;
}
...
void ErrorStateKalmanFilter::CorrectErrorEstimation(
  ...
  case MeasurementType::POSE_VEL:
    //
    // TODO: register new correction logic here:(迭代？)
  ...
  case MeasurementType::POSI_VEL:
    ...
    break;
  case MeasurementType::POSE_VEL_OMC:
    CorrectErrorEstimationPoseWithMC(measurement.T_nb, measurement.v_b, measurement.w_b, Y, G, K);
    break;
  default:
    break;
  }
...
void ErrorStateKalmanFilter::EliminateError(void) {
  ...
  // c. orientation:
  // 原始方法
  Eigen::Matrix3d delta_R = Eigen::Matrix3d::Identity() - Sophus::SO3d::hat(X_.block<3, 1>(kIndexErrorOri, 0)).matrix();
