...
bool KITTIFiltering::Correct(
    ...
    // cloud_pose.block<3, 3>(0, 0) = tmp * cloud_pose.block<3, 3>(0, 0);
    pcl::transformPointCloud(*cloud_data.cloud_ptr, *current_scan_ptr_, cloud_pose);
...
bool KITTIFiltering::InitFusion(const YAML::Node& config_node) {
    ...
    CONFIG.FUSION_STRATEGY_ID["posi_velocity"] = KalmanFilter::MeasurementType::POSI_VEL;
    CONFIG.FUSION_STRATEGY_ID["pose_velocity_with_motion_constraint"] = KalmanFilter::MeasurementType::POSE_VEL_OMC;
...
bool KITTIFiltering::SetInitScan(const CloudData& init_scan) {
    ...
    Eigen::Matrix3f tmp;
    tmp << 0, 1, 0, -1, 0, 0, 0, 0, 1;
    init_pose.block<3, 3>(0, 0) = tmp * init_pose.block<3, 3>(0, 0);
