...
bool KITTIFilteringFlow::SaveOdometry(void) {
    ...
    // std::ofstream ref2fused_vel_ofs;
    ...
    if (
        ...
        // ||!FileManager::CreateFile(ref2fused_vel_ofs, WORK_SPACE_PATH + "/slam_data/trajectory/ref2fused_vel.txt")
    )
    {
        return false;
    }
    ...
    for (size_t i = 0; i < trajectory.N; ++i) {
        ...
        // Eigen::Vector3f tmp_vel = trajectory.fused_vels_.at(i);
        // ref2fused_vel_ofs << tmp_vel(0, 0) << " " << tmp_vel(1, 0) << " " << tmp_vel(2, 0) << std::endl;
    }
...
bool KITTIFilteringFlow::InitLocalization(void) {
    ...
    // Eigen::Vector3f init_vel(0.0, 0.0, 0.0);
...
bool KITTIFilteringFlow::PublishLidarOdom() {
    ...
    laser_pose_.block<3, 3>(0, 0) = tmp * laser_pose_.block<3, 3>(0, 0);
    // _________________________________________ //
...
bool KITTIFilteringFlow::PublishFusionOdom() {
    ...
    Eigen::Matrix3f tmp;
    tmp << 0, -1, 0, 1, 0, 0, 0, 0, 1;
    fused_pose_.block<3, 3>(0, 0) = tmp * fused_pose_.block<3, 3>(0, 0);
    // _________________________________________ //
...
bool KITTIFilteringFlow::UpdateOdometry(const double &time) {
    ...
    // Eigen::Vector3f v_b = (fused_pose_.block<3,3>(0,0)).transpose() * fused_vel_;
    // trajectory.fused_vels_.push_back(v_b);
