...
bool MatchingFlow::UpdateMatching() {
    if (!matching_ptr_->HasInited()) {
        ...
        //
        std::cout << "[DEBUG] current_gnss_data_ : \n"
                  << current_gnss_data_.pose << "; \n"
                  << current_gnss_data_.time
                  << std::endl;
        ...
        //     }
        // }
        // current_gnss_data_.pose.block<3, 3>(0, 0) = lidar_to_imu_.block<3, 3>(0, 0) * current_gnss_data_.pose.block<3, 3>(0, 0);

        // matching_ptr_->SetGNSSPose(current_gnss_data_.pose);

         matching_ptr_->SetScanContextPose(current_cloud_data_);

        //  if (!matching_ptr_->SetGNSSPose(current_gnss_data_.pose) && !matching_ptr_->SetScanContextPose(current_cloud_data_))
        //  {
